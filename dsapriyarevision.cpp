...
for(int i=1;i<n;i++){
      ...
      cout<<max<<endl;
// sorting kimtime complexity O(nlogn) hai jo uppeer wale code se jayda jhai jiska tc O(n) hai
}
...
int binarysearch(vector<int>nums, int target){
    ...
    int right=n-1;
    while(left<=right){
        int mid=left+(right-left)/2;  // for small value (left+right)/2
        if(nums[mid]==target){
            return mid;
        }
...
// n < 2 matlab number 0 ya 1 hua,
// dono prime nahi hote,
bool isprime(int n){
    if(n<2)return false;
    if(n==2)return true;
